...
const NTWRK_PER_K9 = 1000
const WEEKLY_INTEREST_RATE = 10
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      const lockedFormatted = lockedK9s.map((l: any) => ({
        id: l.mint,
        name: `K9 #${l.mint.slice(0, 4)}`,
        image: '/solana-k9s-icon.png',
        locked: true,
        lockDate: l.lockDate,
  ...
  const handleLock = async () => {
    ...
    try {
      const res = await fetch('/api/k9/lock', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ owner: publicKey.toString(), mints: selectedNfts })
      });
      
      const { transaction, blockhash, lastValidBlockHeight, error } = await res.json();
      if (error) throw new Error(error);

      // Reconstruction and Signing
      const tx = Transaction.from(Buffer.from(transaction, 'base64'));
      const signedTx = await signTransaction(tx);

      // Submit with Preflight ENABLED to catch logic errors early
      const signature = await connection.sendRawTransaction(signedTx.serialize(), { 
        skipPreflight: false, 
        preflightCommitment: 'confirmed' 
      });
      
      // Confirm using exact blockhash state from the API
      ...
      console.error(e);
      toast.error(e.message || "Transaction failed - Check your SOL for fees.");
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {k9Nfts.filter(n => !n.locked).map(nft => (
                  <div key={nft.id} onClick={() => setSelectedNfts(prev => prev.includes(nft.id) ? prev.filter(i => i !== nft.id) : [...prev, nft.id])} className={`p-3 rounded-[2rem] border-2 transition-all cursor-pointer bg-gray-900/40 backdrop-blur-md ${selectedNfts.includes(nft.id) ? 'border-blue-500 scale-95' : 'border-white/5'}`}>
                    ...
                    <div className="mt-2 bg-green-500/10 py-1.5 rounded-xl text-center"><p className="text-[10px] font-black text-green-500">+{NTWRK_PER_K9} NTWRK</p></div>
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {k9Nfts.filter(n => n.locked).map(nft => (
                  <div key={nft.id} className="p-3 rounded-[2rem] bg-gray-900/20 border border-white/5 transition-all">
                    <div className="relative aspect-square rounded-2xl overflow-hidden mb-3 grayscale opacity-40"><Image src={nft.image} alt="k9" fill className="object-cover" /></div>
                    <p className="text-[10px] font-black text-gray-500 truncate text-center">{nft.name}</p>
                    <div className="mt-2 bg-orange-500/5 p-2 rounded-xl text-center"><p className="text-sm font-black text-orange-500">{nft.unlockCost} NTWRK</p></div>
      ...
      {showLockModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-white/5 p-10 rounded-[3rem] max-w-sm w-full shadow-2xl text-center">
            ...
            <p className="text-gray-500 text-sm mb-8">Receive <span className="text-green-600 font-bold">{selectedNfts.length * NTWRK_PER_K9} $NTWRK</span> instantly.</p>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-white/5 p-10 rounded-[3rem] max-w-sm w-full text-center shadow-2xl">
            ...
            <div className="bg-orange-500/10 p-8 rounded-[2rem] my-8 border border-orange-500/20 text-center">
                <p className="text-4xl font-black text-orange-600">{selectedForUnlock.unlockCost} <span className="text-sm">NTWRK</span></p>
            </div>
